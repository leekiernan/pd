...
default_opts = [["limitcount","5"],["sortvalue","popularity"],["streamtype","torrent"],["filename","true"]]
...
def oauth(code=""):
    if code == "":
        response = get('https://api.orionoid.com?keyapp='+client_id+'&mode=user&action=authenticate')
        return response.data.code, response.data.code
    else:
        response = None
        while response == None:
            response = get('https://api.orionoid.com?keyapp='+client_id+'&mode=user&action=authenticate&code=' + code)
...
def scrape(query, altquery):
    ...
    if regex.search(r'(tt[0-9]+)', query, regex.I):
        url = 'https://api.orionoid.com?token='+token+'&mode=stream&action=retrieve&type='+type+'&idimdb='+query[2:] + '&' + opts
    else:
        url = 'https://api.orionoid.com?token='+token+'&mode=stream&action=retrieve&type='+type+'&query='+query + '&' + opts
    response = get(url)
    ...
    try:
        ...
        ui_print("[orionoid] matched query: '" + query + "' to " + type + " '" + match + "' - found " + str(response.data.count.total) + " releases (total), retrieved " + str(response.data.count.retrieved) ,ui_settings.debug)
    except:
        None
